...
public class ArbolBinarioDeBusqueda<T extends Comparable<T>> {
    ...
    private Nodo<T> quitar(Nodo<T> nodo, T valor) {
        if (nodo == null) return null;            // no estaba
        int cmp = valor.compareTo(nodo.valor);
        ...
        if (nodo.izquierdo == null) return nodo.derecho;    // 2) un solo hijo:
        if (nodo.derecho == null)   return nodo.izquierdo;  //    el hijo lo reemplaza
    ...
    public List<T> porNiveles() {
        ...
        while (!cola.isEmpty()) {
            ...
            resultado.add(nodo.valor);
            if (nodo.izquierdo != null) cola.addLast(nodo.izquierdo);
            if (nodo.derecho != null)   cola.addLast(nodo.derecho);
        }
        return resultado;
    }
    ...
    public static void main(String[] args) {
        ...
        for (int v : new int[]{50, 30, 70, 20, 40, 60, 80}) abb.insertar(v);
